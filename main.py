...
def send_schedule_teacher(data, user_id, file_name, find_obj):
        ...
        carousel_elements.append(
            {
                ...
                "buttons": [
                    {"action": {"type": "text", "label": item["Группа: "]}}
                ],  # Кнопка под карточкой
            }
        )

    carousel_template = {"type": "carousel", "elements": carousel_elements}
    ...
    vk_session.method(
        "messages.send",
        {
            ...
            "template": json.dumps(
                carousel_template
            ),  # Преобразуем структуру карусели в JSON-строку
            "random_id": 0,
        },
    )
        ...
        carousel_elements.append(
            {
                ...
                # 'action': {'type': 'open_link', 'link': 'https://vk.com'},  # Действие при нажатии на карточку
                "buttons": [
                    {"action": {"type": "text", "label": teacher}}
                ],  # Кнопка под карточкой
            }
        )
                        ...
                        if get_value("user_states", id) == "enter_date":
                            if len(msg) <= 4 and msg[:3].isdigit():
                                ...
                                if "понедельник" in file_name:
                                    schedules = get_schedule_for_group_monday(
                                        file_name + ".xlsx", msg
                                    )
                                else:
                                ...
                                file_name = get_value("file_name", id)
                                if "понедельник" in file_name:
                                    schedules = get_schedule_for_teacher_monday(
                                        file_name + ".xlsx", msg.title()
                                    )
                                else:
                                    schedules = get_schedule_for_teacher(
                                        file_name + ".xlsx", msg.title()
                                    )
                                if schedules:
